...
use super::oauth;
use super::utils::{build_http_client_with_mtls, get_model, ImageFormat}; // Added build_http_client_with_mtls
...
impl DatabricksProvider {
    pub fn from_env(model: ModelConfig) -> Result<Self> {
        ...
        let host = host?;
        let timeout_secs: u64 = config.get_param("DATABRICKS_TIMEOUT").unwrap_or(600);

        let client = build_http_client_with_mtls(&config, "DATABRICKS", timeout_secs)?;
