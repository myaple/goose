...
use crate::providers::utils::build_http_client_with_mtls; // Added
use anyhow::Result;
use mcp_core::tool::{Tool, ToolCall};
use mcp_core::Content;
use reqwest::Client;
use serde_json::{json, Value};
// Removed File, Read, std::time::Duration
use uuid::Uuid;
...
impl OllamaInterpreter {
    pub fn new() -> Result<Self, ProviderError> {
        ...
        // Assuming a default timeout or a specific OLLAMA_TIMEOUT for the interpreter
        let timeout_secs = config.get_param("OLLAMA_INTERPRETER_TIMEOUT").unwrap_or(60); // Shorter timeout for interpreter?

        let client = build_http_client_with_mtls(&config, "OLLAMA", timeout_secs)
            .map_err(|e| ProviderError::ExecutionError(format!("Failed to build HTTP client for ToolShim: {}", e)))?;
