...
use crate::providers::utils::{build_http_client_with_mtls, emit_debug_trace}; // Added
...
impl GcpVertexAIProvider {
    ...
    async fn new_async(model: ModelConfig) -> Result<Self> {
        ...
        let timeout_secs: u64 = config.get_param("GCP_TIMEOUT").unwrap_or(DEFAULT_TIMEOUT_SECS);

        let client = build_http_client_with_mtls(&config, "GCP", timeout_secs)?;
