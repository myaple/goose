...
impl OpenRouterProvider {
    pub fn from_env(model: ModelConfig) -> Result<Self> {
        ...
        if let (Ok(cert_path), Ok(key_path)) = (
            config.get_param::<String>("OPENROUTER_CLIENT_CERTIFICATE_PATH"),
            config.get_param::<String>("OPENROUTER_CLIENT_KEY_PATH"),
        ) {
        ...
        // Load CA certificate
        if let Ok(ca_path) = config.get_param::<String>("OPENROUTER_CERTIFICATE_AUTHORITY_PATH") {
            if !ca_path.is_empty() {
